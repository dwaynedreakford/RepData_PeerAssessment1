...
The **mean number of steps taken per day** = `r meanDailyTotal`

The **median number of steps taken per day** = `r medianDailyTotal`
...
The **interval containing the most steps (on average)** is: `r maxInterval`
...
```
There are **`r numMissingValues` missing step measurements** in the data set provided.

```{r}
medianSteps <- median(stepData$steps, na.rm = TRUE)
```
The **median # of steps, calculated across all measurements taken** (with missing values ignored), is: **`r medianSteps`**
...
*With imputed values*, the **mean number of steps taken per day** = `r meanDailyTotal2`

*With imputed values*, the **median number of steps taken per day** = `r medianDailyTotal2`
